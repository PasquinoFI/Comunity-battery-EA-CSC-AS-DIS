...
def scheduling_plot(title,S,T,SoE,SoE_max,P,DP,PVs,Load,bess_bus,Ab,EP,fromdatasaved=False):
    ...
    ax4.set_xticks(x[::6], labels=hours[::6])
    ...
    ax1.set_xticks(x[::6], labels=hours[::6])
    
    for s in range(S):
        ax2.plot(x,Load[str(s)]*Ab)
    ax2.grid()
    ax2.set_ylabel('Load [Wh]')
    ax2.set_xticks(x[::6], labels=hours[::6])
    
    for y in P0:
        ax5.plot(x,y)
    ax5.grid()
    ax5.set_ylabel('GCP [Wh]')
    ax5.set_xticks(x[::6], labels=hours[::6])
    
    ax6.plot(x,DP*Ab)
    ax6.set_ylabel('DP [Wh]')
    ax6.grid()
    ax6.set_xticks(x[::6], labels=hours[::6])
    
    ax3.plot(x,EP*1000)
    ax3.grid()
    ax3.set_ylabel('EP [€/kWh]')
    ax3.set_xticks(x[::6], labels=hours[::6])
