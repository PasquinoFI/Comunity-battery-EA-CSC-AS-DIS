...
funziona solo per le simulazioni e utilizza PV forecast e non PV forecast2, ovvero, non sfruttra i dati storici dei PV
della micrgrid per le pv forecast ne per le fake realisations
"""

import pandas as pd
import numpy as np
from datetime import datetime, timedelta
from PV_forecast import pv_production
...
class controller:
    ...
    def __init__(self,grid,bess,load,PVs,economic,simulation):
        ...
        self.frs = simulation['fake_realisation_scenario']     
    ...
    def pv_lt_forecast(self):
        ...
        for pv in self.PVs:
            for s in range(self.S):
                if s == 0:
                    self.PVs[pv]['pro'] = -pv_production(s,self.time_scheduling,self.Hor,[self.PVs[pv]['tilt']],[self.PVs[pv]['azimuth']],[self.PVs[pv]['kWp']],[self.PVs[pv]['eta_tot']])
                else:
                    self.PVs[pv]['pro'] = pd.concat([self.PVs[pv]['pro'],-pv_production(s,self.time_scheduling,self.Hor,[self.PVs[pv]['tilt']],[self.PVs[pv]['azimuth']],[self.PVs[pv]['kWp']],[self.PVs[pv]['eta_tot']])],axis=1)                                     
        for s in range(self.S):
    ...
    def pv_st_forecast(self):
        self.p_pv_st = self.p_pv.loc[self.time_scheduling][self.frs] # fake realisation scenario constant over control time
        self.q_pv_st = self.q_pv.loc[self.time_scheduling][self.frs] # fake realisation scenario constant over control time
    ...
    def pv_realisation(self):
        self.p_pv_re = self.p_pv.loc[self.time_scheduling][self.frs]  # pv realisatiton queste dovranno venire dalla PMU
        self.q_pv_re = self.q_pv.loc[self.time_scheduling][self.frs]  # pv realisation queste dovranno venire dalla PMU
    ...
    def run_simulation(self):
        ...
        while self.time_scheduling <= self.time_end:
            ...
            while self.time_control < self.time_scheduling+timedelta(hours=1): # one hour of real time control
                ...
                self.pv_st_forecast() # per ora costanti potrebbero stare fuori
                self.pv_realisation() # per ora costanti potrebbero stare fuori
...
if __name__=='__main__': ###########################################################################
    ...
    PVs = {'PVFacade': {'bus':6, 'tilt':90, 'azimuth':180, 'kWp':14, 'eta_tot':0.85},
           'Solarmax': {'bus':8, 'tilt':10, 'azimuth':180, 'kWp':16, 'eta_tot':0.85},
           'Perun':    {'bus':8, 'tilt':10, 'azimuth':180, 'kWp':13, 'eta_tot':0.70}}
    economic = {'ep_filename':'PUN234.csv', 'acost':0.15, 'cscinc':0.12, 'unbcost':0.3}
    simulation = {'scenarios':20, 'load_fake_realisation_filename':'12_residential_load_profiles_s1_seconds.csv', 'fake_realisation_scenario':1, 
                  'horizon':24, 'Ks':1, 'Kc':1, 'rtc_step':10, 'start':'2024-06-01 00:00:00', 'end':'2024-06-01 23:00:00'}
