...
class controller:
    ...
    def load_st_forecast_exp(self):
        self.p_load_st = [0]*self.B
        self.q_load_st = [0]*self.B
        self.p_load_st[self.loadbus] = self.P_rtcstep[self.time_st_forecast:].mean()[self.loadbus]  # moving average 
        self.q_load_st[self.loadbus] = self.Q_rtcstep[self.time_st_forecast:].mean()[self.loadbus]  # moving average
        ...
        while self.pushing:
            try:
                ...
                if not self.manual_control: # resources contolled by optimisations 
                    ...
                    Franci = { 
                        'ZENONE' : {"P_set [kW]": self.manual['ZENONE']["P_set [kW]"], 
                                    "Q_set [kVAr]": self.manual['ZENONE']["Q_set [kVAr]"]},
                         'BESS' : {"P_set [kW]": -self.manual['BESS']["P_set [kW]"], 
                                   "Q_set [kVAr]": -self.manual['BESS']["Q_set [kVAr]"]}}                    
...
# =============================================================================
    grid = {'filename':'linedata_AC_test.txt', 'Ab':60000, 'Eb':400}
    bess = {'bus':6, 'r':0.32, 'SoE_max':60000, 'SoE':30000, 'P_max':20000}
    load = {'bus':2, 'filename':'12_residential_load_profiles_40s_hourly.csv'}
    PVs = {'PVFacade': {'bus':8, 'filename_data_h':'PV_PVFacade realisation_h.csv', 'filename_data':'PV_PVFacade realisation.csv'},
           'Solarmax': {'bus':8, 'filename_data_h':'PV_Solarmax realisation_h.csv', 'filename_data':'PV_Solarmax realisation.csv'}}
    ...
    simulation = {'scenarios':40, 'load_fake_realisation_filename':'12_residential_load_profiles_s1_5minutes.csv', 'bidding_lag':1,
                  'horizon':24, 'Ks':1, 'Kc':1, 'rtc_step':30, 'start':'2024-06-17 00:00:00', 'end':'2024-06-17 01:00:00'}
    ...
    a.pull_push_test(seconds=60,p_bess=2000) # [W]
